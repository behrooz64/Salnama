...
function renderAnimals(){let box=document.getElementById('animals');animals.forEach((a,i)=>{let b=document.createElement('button');b.className='animal-card';b.innerHTML=`${a.emoji}<br>${a.fa}<br><small>${a.tk}</small>`;b.onclick=()=>{document.querySelectorAll('.animal-card').forEach(x=>x.classList.remove('active'));b.classList.add('active');showAnimalYears(i)};box.appendChild(b);});updateReferenceDisplay();}
