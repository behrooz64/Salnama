...
function showAnimalYears(index){let result=document.getElementById('result');let html=`<h2>${animals[index].emoji} ${animals[index].fa}</h2><p>انتخاب سن بر اساس سال حیوانی:</p><div class="age-list">`;for(let year=referenceYear;year>=referenceYear-150;year--){if(animalIndex(year)===index){let age=referenceYear-year;html+=`<button class="age-item" onclick="showResult(${index},${year})">${age} سال</button>`;}}html+='</div>';result.innerHTML=html;}
